...
def main(file_path):
    ...
    if creation_time and modification_time:
        output.append(f"Creation Time: {creation_time}")
        output.append(f"Modification Time: {modification_time}")

    owner, group, permissions = get_file_owner_permissions(file_path)
    if owner and group and permissions:
        output.append(f"Owner: {owner}, Group: {group}, Permissions: {permissions}")
    ...
    if file_path.lower().endswith(('.jpg', '.jpeg', '.png', '.gif', '.bmp')):
        ...
        if exif_data:
            output.append(f"EXIF Data: {exif_data}")

    if file_path.lower().endswith('.pdf'):
        pdf_metadata = get_pdf_metadata(file_path)
        if pdf_metadata:
            output.append(f"PDF Metadata: {pdf_metadata}")

    with vt.Client(API_KEY) as client:
        is_match = check_file_extension_mismatch(file_path)
        if is_match is not None:
            classify_and_print_file_type(file_path)

        file_hash = get_file_hash(file_path)
        output.append(f"File SHA256 Hash: {file_hash}")

        try:
            report = client.get_object(f"/files/{file_hash}")
            output.append("File scan report found in VirusTotal.")
        except vt.error.APIError:
            output.append("No existing scan report found. Uploading file for a new scan now")
            report = upload_file_to_virustotal(client, file_path)
            output.append("File scanned successfully.")

        # Add malicious report summary
        if report.last_analysis_stats['malicious'] > 0:
            output.append("⚠️ Malicious content detected!")
            for engine, result in report.last_analysis_results.items():
                if result['category'] == 'malicious':
                    output.append(f"- {engine}: {result['result']}")
        else:
            output.append("✅ No known malware detected.")
